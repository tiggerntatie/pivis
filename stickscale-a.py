...
for cm in range (0,101): # for every cm along the stick
    for mm in range (0,10,1): # for every other mm in the cm
        if mm and cm == 100:
            break
        if not mm:
            marklength = 1.2 # 2 cm mark for first
        else:
            marklength = 0.4 # 8 mm mark for others
        xstart = cm + CMW/2.0 - 50.0 + mm/10.0
        yend = CMH-marklength
        ...
        if not mm:
            ...
            ctx.move_to(cm-x_advance + CMW/2.0 - 50.0 - markwidth, ytext)
        elif not mm % 2:
            text = str(mm)
...
surface.write_to_png ("stickscale-a.png") # Output to PNG
